...
const apiKey = "c2dd4c00faa746c74724edfe9877d1bc&units=imperial"
let zipCodeTemp = new Map()
...
function getNasaLocationWeather (location, i){
  // needed all zipcodes to be max 5 digits, used the slice method, start at 0, end at 5(exclusive)
    ...
    console.log(zipCodeTemp)
    console.log(zip)
    console.log(zipCodeTemp.has(zip))
    if(zipCodeTemp.has(zip)){
      let locationWeather = zipCodeTemp.get(zip)
      ...
      let listItem = document.querySelector("#centerName").textContent = `The NASA ${location.center}, ${location.facility} is
      located in ${location.city}, ${location.state} and the temperature today is ${locationWeather}`
    ...
    console.log(`Calling the API for ${zip}`)
...
fetch(`https://api.openweathermap.org/data/2.5/weather?zip=${zip},us&appid=${apiKey}&units=imperial`)
      ...
      .then(weather => {
        ...
        zipCodeTemp.set(zip, locationWeather)
        let li = document.createElement("li") //built in element thats create element so you have a click event that creates a new li; if the input is null, you might want to add something to say a value is entered
        let listItem = document.querySelector("#centerName").textContent = `The NASA ${location.center}, ${location.facility} is
